...
public class StandupTaskTools {
     ...
     *
     * 功能：
     * - 查询指定项目和 Sprint 中，指定用户的所有"进行中"状态的任务
     ...
     * - 用户询问"我负责的任务"
     *
     * @param request 包含项目ID、SprintID 和用户ID的请求参数
    ...
    public String getInProgressTasks(GetInProgressTasksRequest request) {
        log.info("Tool called: getInProgressTasks - projectId: {}, sprintId: {}, userId: {}",
                request.projectId(), request.sprintId(), request.userId());

        try {
            // 查询任务：项目匹配 + Sprint 匹配 + 用户匹配 + 状态为"进行中"
            List<Task> tasks = taskRepository.findByProjectIdAndSprintIdAndAssigneeIdAndStatus(
                    request.projectId(),
                    request.sprintId(),
                    request.userId(),
