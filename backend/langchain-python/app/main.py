...
@app.post("/agent/standup/a2a", response_model=StandupQueryResponse)
async def standup_query_a2a(request: StandupQueryRequest, force_a2a: bool = False):
    """A2A 通信模式站会问答接口
    ...
    """
    from .a2a.workflow import run_a2a_workflow
    
    logger.info("=" * 80)
    logger.info(f"[A2A] Request received - question: {request.question}")
    logger.info(f"[A2A] projectId={request.projectId}, sprintId={request.sprintId}, userId={request.userId}")
    
    try:
        t0 = time.time()
        
        result = await run_a2a_workflow(
            question=request.question,
            project_id=request.projectId,
            sprint_id=request.sprintId,
            user_id=request.userId,
        ...
        # 将 a2a_trace 转换为字符串格式
        a2a_trace = result.get("a2a_trace", [])
        evidence = [f"{t['caller']} → {t['callee']}: {t['reason']} ({t['duration_ms']}ms)" for t in a2a_trace]
        ...
        logger.info(f"[A2A] Response generated in {total_ms}ms")
        logger.info(f"[A2A] A2A calls: {len(result.get('a2a_trace', []))}")
        logger.info("=" * 80 + "\n")
        ...
        logger.error("=" * 80)
        logger.error(f"[A2A] Error occurred: {e}")
        logger.error("=" * 80 + "\n")
        raise
