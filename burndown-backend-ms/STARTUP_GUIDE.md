...
## 一、基础设施（部署在公网服务器 159.75.202.106）

> 以下服务均部署在远程服务器 `159.75.202.106`，本地无需安装，只需确保防火墙端口已开放。

| 组件 | 公网地址 | 端口 | 默认凭据 |
|------|----------|------|----------|
| PostgreSQL | 159.75.202.106 | 5432 | postgres/postgres |
| Redis | 159.75.202.106 | 6379 | 无密码 |
| RabbitMQ | 159.75.202.106 | 5672 | guest/guest |
| Nacos | 159.75.202.106 | 8848 | nacos/nacos |

### 服务器端初始化（首次部署时在服务器上执行）

#### 初始化数据库
```bash
# 在服务器上执行，或本地通过 psql -h 159.75.202.106 连接
psql -h 159.75.202.106 -U postgres -c "CREATE DATABASE burndown_db;"

# 初始化所有 Schema
cd burndown-backend-ms/sql
psql -h 159.75.202.106 -U postgres -d burndown_db -f init-all.sql
```

#### 验证基础设施连通性（本地执行）
```bash
# PostgreSQL
psql -h 159.75.202.106 -U postgres -d burndown_db -c "SELECT 1;"

# Redis
redis-cli -h 159.75.202.106 ping

# RabbitMQ 管理界面
open http://159.75.202.106:15672   # guest/guest

# Nacos 管理界面
open http://159.75.202.106:8848/nacos  # nacos/nacos
```

### 服务器端启动脚本（在 159.75.202.106 上）
```bash
# PostgreSQL
